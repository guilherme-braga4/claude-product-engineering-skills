...
1. Use o **Rovo MCP da Valeti**, preferindo `rovo-valeti` quando disponível. Valide a identidade autenticada e o site antes de qualquer escrita. Não reutilize a identidade ou os recursos de outro conector Atlassian. “Minhas tarefas” significa `assignee.accountId` igual ao accountId confirmado do usuário desta execução, nunca reporter, criador, nome de exibição ou autor do commit.
...
- Confira `git status`, branch e worktrees antes de escrever. Reutilize a branch existente somente se corresponder à Task e à base correta. Preserve alterações alheias; não renomeie branches remotas, force-push, apague branches ou feche PRs históricos automaticamente para corrigir o padrão.
...
- Dois worktrees não podem fazer checkout simultâneo da mesma branch. Dentro de um pai/repositório, serialize escritores na branch compartilhada ou use worktrees em detached HEAD com ownership de arquivos e integração serial pelo responsável. Não use `--force` nem invente branches por Subtask/agente. Branches de integração como `master`/`homolog` mantêm seus nomes e seu papel.
