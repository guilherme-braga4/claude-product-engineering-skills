...
Em entregas da Valeti, inclua [valeti-jira-delivery](../valeti-jira-delivery/SKILL.md) no handoff de implementação: item do ROADMAP → Task pai via Rovo MCP → TASKPLAN → Subtasks, antes de código, usando a branch do pai. A elaboração deste snapshot não cria cards nem altera o Board. A execução autorizada deve nomear um agente para sincronizar somente os cards atribuídos ao usuário confirmado.
