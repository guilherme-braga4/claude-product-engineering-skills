...
Se este for um projeto da Valeti, aplique /valeti-jira-delivery: primeiro leia/crie as Tasks pais pelo Rovo MCP com o nome de cada item do ROADMAP; depois leia/crie o TASKPLAN atual e uma Subtask por item sob o pai correto. Antes de código, verifique os cards e use a branch compartilhada <prefixo>/<CHAVE-DA-TASK-PAI> (ex.: feature/VWAD-1234), preservando o prefixo do Bitbucket e reutilizando o PR por pai/repositório. Não crie branches/PRs por Subtask. Designe um agente responsável por sincronizar status/colunas ao longo da execução e ao fim do desenvolvimento, somente dos cards mapeados atribuídos ao meu accountId confirmado, relendo o responsável antes de cada escrita. Review/merge ou outros gates obrigatórios pendentes impedem DONE. Persista vínculos, evidências, estados confirmados e falhas de sincronização para retomada sem duplicatas.
